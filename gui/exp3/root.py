...
class EXP3Tab(Frame):
    ...
    def _create_widgets(self):
        ...
        # Separator
        border2 = Frame(left_frame, width=500, height=1, background="white")
        border2.grid(row=7, column=0, columnspan=4, pady=10)

        # Subject Name field
        label_subject_name = Label(left_frame, text="Subject Name", background="white", foreground="black", font=("Arial", 12))
        label_subject_name.grid(row=8, column=0, pady=5, padx=5)

        self.entry_subject_name = Entry(left_frame, width=20, relief=SOLID)
        self.entry_subject_name.insert(0, self.subject.subject_name)
        self.entry_subject_name.grid(row=8, column=1, columnspan=2, pady=5, padx=5, sticky="w")

        # Date field
        label_date = Label(left_frame, text="Date", background="white", foreground="black", font=("Arial", 12))
        label_date.grid(row=9, column=0, pady=5, padx=5)

        self.entry_date = Entry(left_frame, width=20, relief=SOLID)
        self.entry_date.insert(0, datetime.now().strftime("%Y-%m-%d"))
        self.entry_date.grid(row=9, column=1, columnspan=2, pady=5, padx=5, sticky="w")
    ...
    def _save(self):
        ...
        self.subject.subject_name = self.entry_subject_name.get()
        self.subject.date = self.entry_date.get()
        self.subject.save()
